...
import { fetchTelemetryHistory } from "@/data/adapters/telemetry-history";
...
export function TelemetryProvider() {
  ...
  // Priority: 1) Recorded history JSON (free, no CORS)
  ...
  useEffect(() => {
    ...
    if (telemetry.horizons && splashdownDate) {
      ...
      // Try recorded history first (generated by PHP cron)
      fetchTelemetryHistory(signal)
        .then((history) => {
          if (signal.aborted || targetIdRef.current !== activeTarget.id) return;

          if (history.length >= 10) {
            // Use recorded history as trajectory
            const now = new Date();
            const past = history.filter((s) => s.timestamp.getTime() <= now.getTime());
            const future = history.filter((s) => s.timestamp.getTime() > now.getTime());
            setTrajectory({ past, future });
            return;
          }

          // Not enough recorded data — fall back to Horizons
