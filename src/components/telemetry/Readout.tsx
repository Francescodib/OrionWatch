...
export function Readout({ label, value, unit, accent = "cyan", icon }: ReadoutProps) {
  return (
    <div className="flex items-baseline justify-between py-1.5 border-b border-space-border/20 last:border-0">
      <span className="text-[9px] font-heading uppercase tracking-[0.12em] text-text-muted flex items-center gap-1.5">
      ...
      <div className="flex items-baseline gap-1">
        <span className={`font-mono text-[13px] font-bold tabular-nums ${colorMap[accent]}`}>
          {value}
        </span>
        {unit && (
